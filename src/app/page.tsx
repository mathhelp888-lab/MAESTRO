...
export default function Home() {
  ...
  const handleDownloadPdf = () => {
    ...
    const addText = (text: string | string[], options: any, startY: number) => {
      const lines = doc.splitTextToSize(text, usableWidth - (options.x || 0));
    ...
    doc.textWithLink("https://distributedapps.ai/", margin + 20, y, { url: 'https://distributedapps.ai/' });
    ...
    if(executiveSummary) {
      ...
      doc.setFontSize(10);
      doc.setTextColor(80);
      y = addText(executiveSummary.replace(/_|\*|#|`/g, ''), {}, y);
      y += 8;
    }
    ...
    layers.forEach((layer) => {
      ...
      if (layer.status === "pending" || layer.status === 'analyzing') {
        ...
        doc.setFontSize(10);
        doc.setTextColor(80);
        y = addText(layer.threat.replace(/_|\*|#|`/g, ''), {x: margin + 2}, y);
        y += 4;
