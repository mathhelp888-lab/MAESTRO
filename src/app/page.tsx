...
export default function Home() {
  ...
  const handleDownloadPdf = async () => {
    ...
    const addText = (text: string, options: any, startY: number) => {
      const { size, style, markdown, x: customX, align } = options;
      
      doc.setFontSize(size || 10);
      doc.setFont("helvetica", style || 'normal');

      const splitOptions: { maxWidth?: number } = {};
      const textX = customX || margin;
      if (align !== 'center') {
          splitOptions.maxWidth = usableWidth - (textX - margin);
      } else {
          splitOptions.maxWidth = usableWidth;
      }
      ...
      const lines = doc.splitTextToSize(plainText, splitOptions.maxWidth!);
      const textHeight = lines.length * (doc.getLineHeight() / doc.getFont().scaleFactor) * 0.8;
  
      if (startY + textHeight > pageHeight - margin) {
        doc.addPage();
        startY = margin;
      }
      doc.text(lines, textX, startY, { align: align || 'left' });
      return startY + textHeight;
    };
    
    // --- HEADER ---
    y = addText("MAESTRO Threat Analysis Report", { size: 20, style: "bold", align: "center", x: pageWidth / 2 }, y);
    y += 10;
  
    // --- DISCLAIMER & DEVELOPER INFO ---
    doc.setTextColor(150);
    const developedByY = y;
    addText("Developed by:", { size: 8, x: margin }, developedByY);
    doc.setTextColor(60, 120, 255);
    doc.textWithLink("DistributedApps.ai", margin + 55, developedByY, { url: 'https://distributedapps.ai/' });
    y += 8;
    ...
    y = addText(disclaimer, { size: 8 }, y);
    y += 20;

    // --- ARCHITECTURE DESCRIPTION ---
    doc.setTextColor(0);
    y = addText("Analyzed System Architecture", { size: 16, style: "bold" }, y);
    y+= 6;
    doc.setTextColor(80);
    y = addText(currentArchitecture, { size: 10 }, y);
    y += 10;
    ...
    if(executiveSummary) {
      ...
      doc.setTextColor(0);
      y = addText("Executive Summary", { size: 16, style: "bold" }, y);
      y+= 6;
      doc.setTextColor(80);
      y = addText(executiveSummary, { size: 10, markdown: true }, y);
      y += 16;
    }
    ...
    layers.forEach((layer) => {
      ...
      doc.setTextColor(0);
      y = addText(layer.name, { size: 14, style: "bold" }, y);
      y += 4;
  
      if (layer.status === "pending" || layer.status === 'analyzing') {
        doc.setFont("helvetica", "italic");
        doc.setTextColor(150);
        y = addText("Pending AI review...", { size: 10, style: "italic" }, y);
      } else if (layer.status === 'error') {
        doc.setTextColor(200, 0, 0);
        y = addText("An error occurred during analysis.", { size: 10, style: "italic" }, y);
      } else if (layer.threat && layer.mitigation) {
        // Threats
        doc.setTextColor(0);
        y = addText("Identified Threats", { size: 12, style: "bold" }, y);
        doc.setTextColor(80);
        y = addText(layer.threat, { size: 10, markdown: true }, y);
        y += 8;
  
        // Mitigation
        doc.setTextColor(0);
        y = addText("Mitigation Strategy", { size: 12, style: "bold" }, y);
        
        y = addText("Recommendation:", { size: 10, style: "bold", x: margin + 4 }, y);
        doc.setTextColor(80);
        y = addText(layer.mitigation.recommendation, { size: 10, x: margin + 8}, y);
  
        doc.setTextColor(0);
        y = addText("Reasoning:", { size: 10, style: "bold", x: margin + 4}, y);
        doc.setTextColor(80);
        y = addText(layer.mitigation.reasoning, { size: 10, x: margin + 8 }, y);
  
        doc.setTextColor(0);
        y = addText("Caveats:", { size: 10, style: "bold", x: margin + 4 }, y);
        doc.setTextColor(80);
        y = addText(layer.mitigation.caveats, { size: 10, x: margin + 8 }, y);
      }
      y += 10;
    });
